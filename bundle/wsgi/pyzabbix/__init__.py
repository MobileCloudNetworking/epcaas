...
class ZabbixAPI(object):
    def __init__(self,
                 ...
                 timeout=None):
        """
        Parameters: server: Base URI for zabbix web interface (omitting
        /api_jsonrpc.php) session: optional pre-configured requests.Session
        instance use_authenticate: Use old (Zabbix 1.8) style authentication
        timeout: optional connect and read timeout in seconds, default: None
        (if you're using Requests >= 2.4 you can set it as tuple: "(connect,
    ...
    def login(self, user='', password=''):
        ...
        resulting auth token for further commands. If use_authenticate is
        set, it uses the older (Zabbix 1.8) authentication command
        """
    ...
    def do_request(self, method, params=None):
        ...
        # We don't have to pass the auth token
        # if asking for the apiinfo.version
        ...
        if 'error' in response_json:  # some exception
            msg = \
                "Error {code}: {message}, {data} while sending {json}".format(
