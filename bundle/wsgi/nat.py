...
class NatAdapter(ABCServiceAdapter):
    ...
    def add_relation(self, config):
        ...
        exp_params = ['GW_NET_A_IP', 'PGW_U_NET_A_IP', 'GW_NET_MGMT_IP',
                     'CLOUD_MGMT_GW_IP', 'VIRT_NET_A_INTF', 'VIRT_NET_A_GW_IP',
                     'VIRT_NET_A_PGWU_IP', 'STATIC_NUMBER']
    ...
    def remove_dependency(self, config, ext_service=None):
        ...
        2) For the nat you need to call the relation departed script to the switch :

        curl -X POST -H "Content-Type:application/json"
        -d "{\"parameters\":[\"$GW_NET_A_IP\",\"$GW_NET_MGMT_IP\"]}"
        http://127.0.0.1:8390/gw/removeRelation/pgw_u
        :return:
        """

        exp_params = ['GW_NET_A_IP', 'GW_NET_MGMT_IP']
        ...
        request = {"parameters": parameters}
        print "I'm the nat adapter, " \
              "remove_dependency nat service, parameters %s" % parameters

        url = 'http://' + config['floating_ips'].get('mgmt') + \
              ':8390/gw/removeRelation/pgw_u'
        headers = {'content-type': 'application/json'}
        response = requests.post(url, data=json.dumps(request),
                                 headers=headers, proxies=self.proxies)
        ...
        print response.content
        print "I'm the nat adapter, " \
              "remove_dependency nat service, received resp %s" % response
...
if __name__ == '__main__':
    ...
    # nat_ip = '192.168.85.96'  # mgmt ip
    # nat_ip_net_a = '172.19.5.96'

    config = {}
    # config['hostname'] = "nat"
    config['hostname'] = NAT_VM_NAME
    config['ips'] = {'mgmt': nat_ip, 'net_a': nat_ip_net_a}
